...
public class KSMOD_TurningVFXEffect extends AbstractGameEffect
{
    ...
    public KSMOD_TurningVFXEffect(KSMOD_AbstractMagicCard clowCard)
    {
        this.startingDuration = ENLARGE_INTERVAL + PARTICLE_GATHER_INTERVAL + SWITCH_INTERVAL + PARTICLE_DIFFUSION_INTERVAL + WAITING_INTERVAL;
        ...
        this.sakuraCard = (KSMOD_AbstractMagicCard) clowCard.cardsToPreview.makeCopy();
    ...
    public void update()
    {
        if (this.duration == this.startingDuration)
        {
            if (doneArray[0])
            {
                doneArray[0] = false;
                // 放大卡片
                AbstractDungeon.effectsQueue.add(new KSMOD_TurningScalingEffect(clowCard, startPosition, targetPosition, START_SCALE, TARGET_SCALE, ENLARGE_DURATION));
            }
        }
        else if (this.duration < this.startingDuration - GetTimeLinePoint(1) && this.duration > this.startingDuration - GetTimeLinePoint(2))
        {
            if (doneArray[1])
            {
                doneArray[1] = false;
                // 显示clow card
                AbstractDungeon.effectsQueue.add(new KSMOD_TurningShowCardEffect(clowCard, targetPosition, TARGET_SCALE, PARTICLE_GATHER_DURATION));
                // 粒子聚集
                AbstractDungeon.effectsQueue.add(new KSMOD_TurningParticleGatherEffect(targetPosition, PARTICLE_GATHER_DURATION));

            }
        }
        else if (this.duration < this.startingDuration - GetTimeLinePoint(2) && this.duration > this.startingDuration - GetTimeLinePoint(3))
        {
            if (doneArray[2])
            {
                doneArray[2] = false;
                // 显示sakura card
                AbstractDungeon.effectsQueue.add(new KSMOD_TurningShowCardEffect(sakuraCard, targetPosition, TARGET_SCALE, SWITCH_DURATION + WAITING_DURATION));
                // 转变
                AbstractDungeon.effectsQueue.add(new KSMOD_TurningSwitchCardEffect(clowCard, targetPosition, SWITCH_DURATION));
                // 迷雾显示
        ...
        else if (this.duration < this.startingDuration - GetTimeLinePoint(3) && this.duration > this.startingDuration - GetTimeLinePoint(4))
        {
            if (doneArray[3])
            {
                doneArray[3] = false;
                // 等待展示
                AbstractDungeon.effectsQueue.add(new KSMOD_TurningShowCardEffect(sakuraCard, targetPosition, TARGET_SCALE, WAITING_DURATION));
                // 粒子扩散
                AbstractDungeon.effectsQueue.add(new KSMOD_TurningParticleDiffusionEffect(targetPosition, PARTICLE_DIFFUSION_DURATION));
            }
        }
        else if (this.duration < this.startingDuration - GetTimeLinePoint(4) && this.duration > 0)
        {
            if (doneArray[4])
            {
                doneArray[4] = false;
                // 卡片缩小
                AbstractDungeon.effectsQueue.add(new KSMOD_TurningScalingEffect(sakuraCard, targetPosition, startPosition, TARGET_SCALE, START_SCALE, SHRINK_DURATION));
            }
        }
        // 特效结束
