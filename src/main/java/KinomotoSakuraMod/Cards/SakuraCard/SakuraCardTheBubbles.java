...
public class SakuraCardTheBubbles extends KSMOD_AbstractMagicCard
{
    ...
    {
        AbstractDungeon.actionManager.addToBottom(new DamageAllEnemiesAction(player, KSMOD_DataTool.GetDamageList(this.damage), DamageInfo.DamageType.NORMAL, AbstractGameAction.AttackEffect.POISON));
    ...
    private void RemoveAllMonsterBuffs(AbstractPlayer player)
    {
        for (AbstractMonster mon : AbstractDungeon.getMonsters().monsters)
        {
            ...
            {
                if (power.type == AbstractPower.PowerType.BUFF && !KSMOD_DataTool.IsStringListContains(ClowCardTheBubbles.powerBlackList, power.ID))
                {
                    buffs.add(power);
                }
            }
