...
public class KSMOD_DrawPileViewScreenPatch
{
    ...
    public static class updatePositions
    {
        public static SpireReturn<Object> Prefix(DrawPileViewScreen deck) throws NoSuchFieldException, IllegalAccessException
        {
            ...
            {
                Field hoveredCard = KSMOD_ReflectTool.GetFieldByReflect(DrawPileViewScreen.class, "hoveredCard");
                hoveredCard.set(deck, null);
                for (int i = 0; i < cards.size(); ++i)
                {
                    AbstractCard card = cards.get(i);
                    int mod = i % CARDS_PER_LINE;
                    Float currentDiffY = KSMOD_ReflectTool.GetFieldByReflect(DrawPileViewScreen.class, "currentDiffY").getFloat(deck);
    ...
    public static class calculateScrollBounds
    {
        public static SpireReturn<Object> Prefix(DrawPileViewScreen deck) throws NoSuchFieldException, IllegalAccessException
        {
            ...
            if (HasLongCard(cards))
            {
                Field scrollUpperBound = KSMOD_ReflectTool.GetFieldByReflect(DrawPileViewScreen.class, "scrollUpperBound");
                ...
                Field prevDeckSize = KSMOD_ReflectTool.GetFieldByReflect(DrawPileViewScreen.class, "prevDeckSize");
