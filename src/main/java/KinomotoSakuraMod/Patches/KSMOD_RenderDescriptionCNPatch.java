...
public class KSMOD_RenderDescriptionCNPatch
{
    ...
    private static final float DES_OFFSET_Y = 0.321F;
            ...
            if (card.hb.hovered)
            {
                if (card.isSeen && !card.isLocked)
                {
                    ...
                    for (int i = 0; i < card.description.size(); ++i)
                    {
                        ...
                        start_x = card.current_x - ((DescriptionLine)card.description.get(i)).width * card.drawScale / 2.0F - 14.0F * Settings.scale;
